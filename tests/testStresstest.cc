...
BOOST_AUTO_TEST_CASE(stresstest) {
  ...
  static_assert(nQueues % nQueuesPerSender == 0,
                "nQueues and nQueuesPerSender do not fit together");
  static_assert(nQueues % nQueuesPerReceiver == 0,
  ...
  for (size_t i = 0; i < nSenders; ++i) {
    ...
    senderThreads.emplace_back([myqueues, &shutdownSenders]() mutable {
      ...
      for (size_t i = 0; i < myqueues.size(); ++i)
      ...
      while (!shutdownSenders) {
        for (size_t k = 0; k < nQueuesPerSender; ++k) {
          ...
          if (success) {
            ...
            if (consequtive_fails[k] > 100)
              usleep(1000);
            if (consequtive_fails[k] > 1000)
              usleep(1000000);
          }
        }
      }
