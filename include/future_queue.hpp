...
struct shared_state_ptr {
  ...
  template <typename T> void make_new(size_t length);
  ...
  template <typename T> shared_state<T> *cast();
...
template <typename T> shared_state_ptr make_shared_state(size_t length) {
  shared_state_ptr p;
  p.make_new<T>(length);
  return p;
}
...
template <typename T, typename FEATURES> class future_queue;
...
protected:
  ...
  template <typename T, typename FEATURES> friend class ::cppext::future_queue;

  template <typename ITERATOR_TYPE>
  friend future_queue<size_t, MOVE_DATA> when_any(ITERATOR_TYPE begin,
                                                  ITERATOR_TYPE end);

  template <typename ITERATOR_TYPE>
  friend future_queue<void, MOVE_DATA> when_all(ITERATOR_TYPE begin,
                                                ITERATOR_TYPE end);
...
public:
  ...
  future_queue<T2, FEATURES2> then(CALLABLE callable,
                                   std::launch policy = std::launch::async);

  friend future_queue<T, FEATURES>
  atomic_load(const future_queue<T, FEATURES> *p) {
  ...
  friend void atomic_store(future_queue<T, FEATURES> *p,
                           future_queue<T, FEATURES> r) {
...
struct shared_state_base {
  ...
  shared_state_base(size_t length)
      : nBuffers(length + 1), semaphores(length + 1), exceptions(length + 1),
        writeIndex(0), readIndexMax(0), readIndex(0), hasFrontOwnership(false),
...
template <typename T> struct shared_state : shared_state_base {
...
template <> struct shared_state<void> : shared_state_base {
...
future_queue<size_t> when_any(ITERATOR_TYPE begin, ITERATOR_TYPE end) {
  ...
  for (ITERATOR_TYPE it = begin; it != end; ++it) {
    ...
    for (size_t i = 0; i < nPreviousValues; ++i)
      notifyerQueue.push(index);
    ++index;
  }
...
future_queue<void> when_all(ITERATOR_TYPE begin, ITERATOR_TYPE end) {
  ...
  notifyerQueue.d->continuation_process_deferred =
      std::function<void(void)>([notifyerQueue, participants]() mutable {
        bool empty = false;
  ...
  notifyerQueue.d->continuation_process_deferred_wait =
      std::function<void(void)>(
          [notifyerQueue, participants, anyNotify]() mutable {
            while (true) {
              anyNotify.pop_wait();
              bool empty = false;
              for (auto &q : participants) {
 ...
 *  for functions). */
template <typename T> void data_assign(T &a, T &&b, MOVE_DATA) {
...
template <typename T> void data_assign(T &a, T &&b, SWAP_DATA) {
  std::swap(a, b);
}
...
operator=(const shared_state_ptr &other) {
...
inline void shared_state_ptr::free() {
  ...
  if (oldCount == 1 && !get()->is_continuation_async &&
      !get()->is_continuation_deferred && !get()->is_continuation_when_all) {
    executeDelete = true;
  }
      ...
      while (get()->continuation_process_async_terminated == false) {
        ...
        try {
          ...
          if (get()->continuation_origin.d->is_continuation_deferred ||
            ...
            get()->continuation_origin.d->continuation_origin.push_exception(
...
template <typename T> void shared_state_ptr::make_new(size_t length) {
  free();
...
template <typename T> shared_state<T> *shared_state_ptr::cast() {
  assert(get() != nullptr);
...
inline bool
future_queue_base::push_overwrite_exception(std::exception_ptr exception) {
  ...
  if (!obtain_write_slot(myIndex)) {
    if (d->semaphores[(myIndex - 1) % d->nBuffers].is_ready_and_reset()) {
      ...
      bool success =
          d->writeIndex.compare_exchange_strong(expectedIndex, myIndex - 1);
      if (!success) {
...
inline bool future_queue_base::empty() {
  if (d->hasFrontOwnership)
    return false;
  if (d->is_continuation_deferred || d->is_continuation_when_all)
...
inline bool future_queue_base::
operator==(const future_queue_base &other) const {
  return d == other.d;
}
...
inline future_queue_base::future_queue_base(
    const detail::shared_state_ptr &d_ptr_)
    : d(d_ptr_) {}
...
inline bool future_queue_base::obtain_write_slot(size_t &index) {
  index = d->writeIndex;
  while (true) {
    if (index >= d->readIndex + d->nBuffers - 1)
...
inline void future_queue_base::update_read_index_max() {
  ...
  if (l_writeIndex >= l_readIndex + d->nBuffers)
    l_writeIndex = l_readIndex + d->nBuffers - 1;
  size_t newReadIndexMax = l_readIndexMax;
  do {
    for (size_t index = l_readIndexMax; index <= l_writeIndex - 1; ++index) {
      if (!d->semaphores[index % d->nBuffers].is_ready())
        break;
...
bool future_queue<T, FEATURES>::pop(U &t) {
  ...
  if (d->hasFrontOwnership ||
      d->semaphores[d->readIndex % d->nBuffers].is_ready_and_reset()) {
    std::exception_ptr e;
    if (d->exceptions[d->readIndex % d->nBuffers]) {
      e = d->exceptions[d->readIndex % d->nBuffers];
    } else {
      detail::data_assign(t,
                          std::move(future_queue_base::d.cast<T>()
                                        ->buffers[d->readIndex % d->nBuffers]),
                          FEATURES());
    }
 ...
 * discarded) */
template <typename T, typename FEATURES> bool future_queue<T, FEATURES>::pop() {
  if ((d->is_continuation_deferred || d->is_continuation_when_all) &&
      !d->hasFrontOwnership) {
    d->continuation_process_deferred();
  }
...
void future_queue<T, FEATURES>::pop_wait(U &t) {
  ...
  if (d->exceptions[d->readIndex % d->nBuffers]) {
    ...
    detail::data_assign(t,
                        std::move(future_queue_base::d.cast<U>()
                                      ->buffers[d->readIndex % d->nBuffers]),
                        FEATURES());
  }
  assert(d->readIndex < d->writeIndex);
...
const U &future_queue<T, FEATURES>::front() const {
  ...
  if (d->exceptions[d->readIndex % d->nBuffers])
    std::rethrow_exception(d->exceptions[d->readIndex % d->nBuffers]);
...
template <typename T, typename FEATURES, typename TOUT, typename CALLABLE>
continuation_process_deferred<T, FEATURES, TOUT, CALLABLE>
make_continuation_process_deferred(future_queue<T, FEATURES> q_input,
                                   future_queue<TOUT> q_output,
                                   CALLABLE callable) {
...
template <typename T, typename FEATURES, typename TOUT, typename CALLABLE>
continuation_process_deferred_wait<T, FEATURES, TOUT, CALLABLE>
make_continuation_process_deferred_wait(future_queue<T, FEATURES> q_input,
                                        future_queue<TOUT> q_output,
                                        CALLABLE callable) {
  return {q_input, q_output, callable};
}
...
template <typename T, typename FEATURES, typename TOUT, typename CALLABLE>
struct continuation_process_async {
  continuation_process_async(future_queue<T, FEATURES> q_input_,
                             future_queue<TOUT> q_output_, CALLABLE callable_)
      : q_input(q_input_), q_output(q_output_), callable(callable_) {}
  void operator()() {
    while (true) {
...
struct continuation_process_async<void, FEATURES, TOUT, CALLABLE> {
  continuation_process_async(future_queue<void, FEATURES> q_input_,
                             future_queue<TOUT> q_output_, CALLABLE callable_)
      : q_input(q_input_), q_output(q_output_), callable(callable_) {}
  void operator()() {
    while (true) {
...
struct continuation_process_async<T, FEATURES, void, CALLABLE> {
  continuation_process_async(future_queue<T, FEATURES> q_input_,
                             future_queue<void> q_output_, CALLABLE callable_)
      : q_input(q_input_), q_output(q_output_), callable(callable_) {}
  void operator()() {
    while (true) {
      // written this way so the callable is able to swap with the internal
...
template <typename T, typename FEATURES, typename TOUT, typename CALLABLE>
continuation_process_async<T, FEATURES, TOUT, CALLABLE>
make_continuation_process_async(future_queue<T, FEATURES> q_input,
                                future_queue<TOUT> q_output,
                                CALLABLE callable) {
  return {q_input, q_output, callable};
}
...
future_queue<T2, FEATURES2>
future_queue<T, FEATURES>::then(CALLABLE callable, std::launch policy) {
  ...
  if (policy == std::launch::deferred) {
    ...
    q_output.d->continuation_process_deferred_wait =
        detail::make_continuation_process_deferred_wait(q_input, q_output,
                                                        callable);
    ...
    q_output.d->continuation_process_async = std::thread(
        detail::make_continuation_process_async(q_input, q_output, callable));
    q_output.d->continuation_origin = *this;
