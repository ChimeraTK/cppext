...
public:
  ...
  /** Destroy semaphore. No other thread must be waiting on this semaphore when
   * the destructor is called. */
  ~semaphore() { sem_destroy(&sem); }

  /** Make the semaphore ready. If another thread is currently waiting for the
   * semaphore (see wait_and_reset()), it
   *  will be unblocked. Calling unlock() on a semaphore which is already ready
   * is not allowed. */
  void unlock() {
  ...
  /** Check if the semaphore is currently ready. Does not block or alter the
  ...
  /** Block the calling thread until the semaphore gets ready, i.e. unlock() is
   * called by another thread. If the semaphore is already ready when entering
   * this function, it will not block. The semaphore will be atomically locked
   * again, so if multiple threads are waiting on the same semaphore at the same
   * time, only one of them will unblock. */
  void wait_and_reset() {
  ...
  /** Check if the semaphore is ready. If not, return false immediately. If yes,
   * atomically lock the semaphore and return true. */
