...
namespace cppext {
  namespace experimental {
    ...
    /**
     *  A simple "type erasing" container which works without predefining a list of allowed types and calls a templated
     *  functor object or C++14 lambda with auto-type parameter. The functor object / lambda gets instantiated when
     ...
     *      std::cout << "HIER " << typeid(T).name() << " -> " << value << std::endl;
     *    }
     *  };
     *
     *  int main() {
    ...
    class type_erasing_vector {
      public:
        ...
        std::function<void(void)> at(size_t index) {
