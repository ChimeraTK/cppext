...
BOOST_AUTO_TEST_CASE(stresstestMultiproducer) {
  ...
  for (size_t i = 0; i < nSenders; ++i) {
    senderThreads.emplace_back([i, q, &shutdownSenders]() mutable {
      ...
      while (!shutdownSenders) {
        ...
        if (success) {
          ++nextValue;
          if (nextValue >= senderFirstValue + idsPerSender)
            nextValue = senderFirstValue;
          consequtive_fails = 0;
        } else {
          ++consequtive_fails;
          BOOST_CHECK_TS(consequtive_fails < 1000);
          if (consequtive_fails > 100)
            usleep(100000);
        }
      }
    }); // end sender thread
  ...
  boost::thread receiverThread([q, &shutdownReceiver]() mutable {
    ...
    while (!shutdownReceiver) {
      ...
      nextValues[senderId]++;
      if (nextValues[senderId] >= ((signed)senderId + 1) * idsPerSender)
        nextValues[senderId] = senderId * idsPerSender;
    }
  }); // end receiver thread

  // run the test for N seconds
  std::cout << "Keep the test running for " << runForSeconds << " seconds..."
            << std::endl;
  sleep(runForSeconds);
